...
Boolean CreateScoresFolder (long *scoresDirID)
{
	...
	theErr = FSMakeFSSpec(volRefNum, prefsDirID, PSTR("G-PRO Scores ƒ"), &scoresSpec);
...
Boolean FindHighScoresFolder (short *volRefNum, long *scoresDirID)
{
	...
	PasStringCopy(PSTR("G-PRO Scores ƒ"), nameString);
	count = 1;
	foundIt = false;
	...
	while ((theErr == noErr) && (!foundIt))
	{
		...
		if (theErr == noErr)
		{
			...
			{
				if (EqualString(theBlock.dirInfo.ioNamePtr, PSTR("G-PRO Scores ƒ"), 
						true, true))
				{
					foundIt = true;
